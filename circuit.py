...
class PriorityQueue:
    ...
    def is_leaf(self, index):
        level_prev = math.floor(math.log2(self.__len__() + 1))
        last_index_of_previous_level = math.pow(2, level_prev) - 2
        if last_index_of_previous_level < index and index <= self.__len__() - 1:
            return True
        else:
            return False
    ...
    def append(self, key):
        if key == None:
            raise ValueError("None cannot be inserted into heap")
        else:
            self.min_heap.append(key)
            self.last_child_index += 1
            self.down_to_up_heapify(self.last_child_index)
    ...
    def pop(self):
        if len(self.min_heap) <= 0:
            ...
            to_pop = self.min_heap[0]
            self.min_heap[0] = self.min_heap[self.last_child_index]
            self.min_heap.pop()
            self.last_child_index -= 1
            self.up_to_down_heapify(0)
            print(self.min_heap)
            return to_pop

    def up_to_down_heapify(self, index):
        element_to_move = index
        if not self.is_leaf(element_to_move):
            left_child = 2 * element_to_move + 1
            right_child = 2 * element_to_move + 2
            if left_child < self.__len__() and right_child < self.__len__():
                if self.min_heap[left_child] < self.min_heap[element_to_move]:
                    # switch left and root
                    self.min_heap[left_child], self.min_heap[element_to_move] = self.min_heap[element_to_move], \
                    self.min_heap[left_child]
                    self.up_to_down_heapify(left_child)
                elif self.min_heap[right_child] < self.min_heap[element_to_move]:
                    # switch right and root
                    self.min_heap[right_child], self.min_heap[element_to_move] = self.min_heap[element_to_move], \
                    self.min_heap[right_child]
                    self.up_to_down_heapify(right_child)
        else:
            return

    def down_to_up_heapify(self, index):
        if index > 0:
            index_of_moving_element = index
            parent_index = self.find_parent(index_of_moving_element)
            if self.min_heap[parent_index] > self.min_heap[index_of_moving_element]:
                self.min_heap[parent_index], self.min_heap[index_of_moving_element] = self.min_heap[
                    index_of_moving_element], self.min_heap[parent_index]
            index_of_moving_element = parent_index
            self.down_to_up_heapify(index_of_moving_element)
        else:
            return
